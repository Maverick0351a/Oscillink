...
def run_benchmark_json(args) -> dict:
    cmd = [sys.executable, 'scripts/benchmark.py', '--json', '--N', str(args.N), '--D', str(args.D), '--kneighbors', str(args.kneighbors), '--trials', str(args.trials)]
...
def main():
    ...
    for m in metrics:
        ...
        if pct > args.tolerance_pct:
            failures.append(f"Metric {m} regressed {pct:.1f}% (baseline={base_mean:.2f}, current={curr_mean:.2f})")
    result = {
