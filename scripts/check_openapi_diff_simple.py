...
def main():
    ...
    ap.add_argument('--allow-removed', type=str, default="", help='Comma list of path patterns to ignore (exact match)')
    args = ap.parse_args()
    prev = load(args.prev)
    ...
    if missing:
        sys.stderr.write("Breaking changes detected (removed paths/methods):\n" + "\n".join(missing) + "\n")
        sys.exit(1)
