...
def main():
    ...
    if args.proof:
        ...
        if args.bundle_k > 0:
            ...
            lat_b = OscillinkLattice(Yb, kneighbors=args.kneighbors, lamG=args.lamG, lamC=args.lamC, lamQ=args.lamQ, deterministic_k=True)
        ...
        if args.diffusion:
            ...
            gates = compute_diffusion_gates(Yd.astype(np.float32), psi_d.astype(np.float32), kneighbors=args.kneighbors, deterministic_k=True)
            lat_u = OscillinkLattice(Yd, kneighbors=args.kneighbors, lamG=args.lamG, lamC=args.lamC, lamQ=args.lamQ, deterministic_k=True)
            lat_u.set_query(psi_d)
            lat_u.settle(max_iters=12, tol=1e-3)
            rec_u = lat_u.receipt()
            lat_d = OscillinkLattice(Yd, kneighbors=args.kneighbors, lamG=args.lamG, lamC=args.lamC, lamQ=args.lamQ, deterministic_k=True)
        ...
        print(f"ΔH={proof_row['deltaH']:.3f}  null_points={proof_row['null_points']} sample_null={proof_row['sample_null']}")
