...
def main():
    ...
    for t in range(args.trials):
        trials.append(run_once(args.N, args.D, args.kneighbors, args.beta, args.gamma, args.bundle_k, args.seed + t))

    summary = summarize(trials)

    if args.json:
        print(json.dumps({'config': vars(args), 'summary': summary, 'trials': trials}, separators=(',', ':'), sort_keys=True))
    else:
