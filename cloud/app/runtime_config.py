...
def get_rate_limit() -> RateLimitConfig:
    try:
        return RateLimitConfig(limit=int(os.getenv("OSCILLINK_RATE_LIMIT", "0")), window=int(os.getenv("OSCILLINK_RATE_WINDOW", "60")))
    except ValueError:
        return RateLimitConfig(limit=0, window=60)

def get_quota_config() -> QuotaConfig:
    try:
        return QuotaConfig(limit=int(os.getenv("OSCILLINK_KEY_NODE_UNITS_LIMIT", "0")), window=int(os.getenv("OSCILLINK_KEY_NODE_UNITS_WINDOW", "3600")))
    except ValueError:
