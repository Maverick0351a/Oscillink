...
T1 = time.time(); lat_d.settle(); settle_d_ms = 1000.0 * (time.time() - T1)
rec_d = lat_d.receipt()
bundle_d = lat_d.bundle(k=8)
mean_align_d = sum(b['align'] for b in bundle_d) / len(bundle_d)

print("=== Gating Comparison ===")
print(f"Uniform   ΔH={rec_u['deltaH_total']:.4f}  mean_align={mean_align_u:.4f}  settle_ms={settle_u_ms:.2f}")
print(f"Diffusion ΔH={rec_d['deltaH_total']:.4f}  mean_align={mean_align_d:.4f}  settle_ms={settle_d_ms:.2f}")
print("Gate stats: min={:.3f} max={:.3f} mean={:.3f}".format(float(gates.min()), float(gates.max()), float(gates.mean())))
