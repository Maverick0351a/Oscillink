...
def oscillink_rag(embeddings: np.ndarray, query_embedding: np.ndarray, k: int = 5) -> Dict[str, Any]:
...
def main():
    ...
    try:
        ...
        print(f"   Gate distribution: min={gates.min():.3f}, max={gates.max():.3f}, mean={gates.mean():.3f}")
